...
/**
 * El siguiente script muestáa las imagenes referenciadas mediante su URL en un
 * array que debe llamarse Pic. Dicho array puede incluirse en un fichero .js
 * que debe cargarse en la página antes que está fichero zcommon.js o generarse
 * din�micamente pero definiendo el array Pic y inicióndolo antes de la carga de
 * zcommon.js.
 * Created by Pierre Volpe
 *
 * Se inicializan unas variables de trabajo al cargar la página y provee una función
 * que debe ser llamada cuando se termina de cargar la página y que muestáa un
 * show continuo de la lista de imagenes precargadas
 * */

var div_inicial ; // destánado a definir en �l el nombre del elemento 'div' que se debe mostrar al cargar la página
//
//
...
//El siguiente script abre la ventana de galer�a de fotos que corresponda seg�n la selección realizada en una lista
function destáno(){
...
//El siguiente script abre una nueva ventana con diferentes tama�os y opciones seg�n  los parámetros que se le pasan
...
// Saca un mensaje con scroll horizontal en la barra de estádo
//texto del mensaje
var texto_estádo = "             La gente se arregla todos los d�as el cabello. �Por qu� no el coraz�n?            Si te caes siete veces, lev�ntate ocho.                  Las grandes almas tienen voluntades; las d�biles tan solo deseos.             El sabio puede sentarse en un hormiguero, pero s�lo el necio se queda sentado en �l.                Cuando te inunde una enorme alegr�a, no prometas nada a nadie. Cuando te domine un gran enojo, no contestás ninguna carta.              Si quieres que algo se haga, enc�rgaselo a una persona ocupada.            Nunca se pierden los a�os que se quita una mujer, van a parar a cualquiera de sus amigas.          El que teme sufrir ya sufre el temor.        El momento elegido por el azar vale siempre más que el momento elegido por nosotros mismos.          Es más f�cil variar el curso de un r�o que el car�cter de un hombre."

var posicion = 0
//funcion para mover el texto de la barra de estádo
function mueve_texto(){
    if (posicion < texto_estádo.length)
        posicion ++;
    else
        posicion = 1;
    string_actual = texto_estádo.substring(posicion) + texto_estádo.substring(0,posicion)
...
//-------------------------------------------------------------------
// Funciones utilizadas en el m�dulo comentariosseleccionar
...
function mostrarCalendario(event){
    ...
    if (calendarioVisible==false) {
        x=event.clientX;
        y=event.clientY;
        document.all("divCalendario").style.left = x-80;
...
function validarFormularioNoticiasSeleccionar(){
    ...
    if (textoCampo == "") {
        alert("El campo 'N� max. de Noticias' debe ser num�rico");
        return false;
    }
    if (textoCampo < 1) {
        alert("El campo 'N� max. de Noticias' debe ser mayor que 0");
        return false;
    }
    ...
    if (textoCampo.length>0) {
        ...
        if (textoCampo == "") {
            alert("El campo 'Palabras a buscar' contiene caracteres no alfanum�ricos");
            return false;
        }
    }
...
function validarFormularioPáginasSeleccionar(){
    //extraemos el valor del campo
    textoCampo = window.document.formFechas.maxreg.value
    //lo validamos como entero
    textoCampo = validarEntero(textoCampo)
    //colocamos el valor de nuevo
    if (textoCampo == "") {
        alert("El campo 'N� max. de Páginas' debe ser num�rico");
        return false;
    }
    if (textoCampo < 1) {
        alert("El campo 'N� max. de Páginas' debe ser mayor que 0");
        return false;
    }
...
function validarFormularioComentariosSeleccionar(){
    ...
    //colocamos el valor de nuevo
    if (textoCampo == "") {
        alert("El campo 'N� max. de comentarios' debe ser num�rico");
        return false;
    }
    if (textoCampo < 1) {
        alert("El campo 'N� max. de comentarios' debe ser mayor que 0");
        return false;
    }
...
function validarFormularioAnunciosSeleccionar(){
    ...
    //colocamos el valor de nuevo
    if (textoCampo == "") {
        alert("El campo 'N� max. de anuncios' debe ser num�rico");
        return false;
    }
    if (textoCampo < 1) {
        alert("El campo 'N� max. de anuncios' debe ser mayor que 0");
        return false;
    }
...
function validarAlfanumerico (texto) {
    var caracteres_validos=" 0123456789ABCDEFGHIJKLMN�OPQRSTUVWXYZabcdefghijklmn�opqrstuvwxyz";
...
// Comprueba que una fecha es v�lida
...
function isDate(dtStr){
	...
	year=parseInt(strYr)
	if (pos1==-1 || pos2==-1){
		alert("El formato del campo 'Fecha' deberáa ser : dd/mm/yyyy")
		return false
	}
	if (strMonth.length<1 || month<1 || month>12){
		alert("Por favor, entre un mes v�lido")
		return false
	}
	if (strDay.length<1 || day<1 || day>31 || (month==2 && day>daysInFebruary(year)) || day > daysInMonth[month]){
		alert("Por favor, entre un d�a v�lido")
		return false
	}
	if (strYear.length != 4 || year==0 || year<minYear || year>maxYear){
		alert("Por favor, entre un mes a�o de 4 d�gitos v�lido entre "+minYear+" y "+maxYear)
		return false
	}
	if (dtStr.indexOf(dtCh,pos2+1)!=-1 || isInteger(stripCharsInBag(dtStr, dtCh))==false){
		alert("Por favor, entre una fecha v�lida")
		return false
	}
return true
}
